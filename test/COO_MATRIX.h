...
public:
	...
	COO_MATRIX(const MATRIX& mat){
		...
		for (int x = 0; x < mat.rows(); x++)
			for (int y = 0; y < mat.cols(); y++){
				if (abs(mat(x, y)) > 1e-16)
					_matrix.push_back(TRIPLET(x, y, mat(x, y)));
				if (x == y)
					_diag[x] = mat(x, y);
			}
	}
	...
	inline int lowerNNz() const {
		...
		for (unsigned int x = 0; x < _matrix.size(); x++)
			if (_matrix[x].row() >= _matrix[x].col())
				cnt++;
		return cnt;
	};
	...
	inline void clear() { _matrix.clear(); _diag.setZero(); };
	...
	void entries(vector<int>& rows, vector<int>& cols, vector<Real>& values) const
	{
	...
	void aggregate(bool chopUpperTriangle = false){
		...
		if (!chopUpperTriangle){
			for (unsigned int arrayPtr = 1; arrayPtr < _matrix.size(); arrayPtr++){
				if (_matrix[arrayPtr].row() == _matrix[cleanedPtr].row() && _matrix[arrayPtr].col() == _matrix[cleanedPtr].col()){
	...
	inline void add(const Real& entry, int row, int col)
	{
		...
		{
			if (_matrix[i].row() == row && _matrix[i].col() == col)
			{
				_matrix[i].value() += entry;
				return;
			}
		}
...
#pragma omp parallel for schedule(static)
	...
	COO_MATRIX& operator-=(const COO_MATRIX& A){
		...
		for (unsigned int x = 0; x < other.size(); x++)
			_matrix.push_back(TRIPLET(other[x].row(), other[x].col(), -other[x].value()));
		_rows = _rows > A.rows() ? _rows : A.rows();
		_cols = _cols > A.cols() ? _cols : A.cols();
		return *this;
	}
